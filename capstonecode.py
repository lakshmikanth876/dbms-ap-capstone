...
ASCII_art = pyfiglet.figlet_format("Online Book Store")
print(ASCII_art)

cnx = pymysql.connect(user='root', password='nagacharan898@', host='localhost', port=3306, database='onlinepurchasedb',
                      autocommit=False)
...
def printItems():
    ...
    print('{:<15s}{:<35s}{:<15s}{:<15s}{:<15s}'.format("ID", "Name", "Stock", "Price",
                                                       "Discount"))  # Section headers formatted to be in a grid
    print(
        "===================================================================================================")  # Just a line to make it look pretty

    for i in range(0, resultCount):  # Make sure it only runs for the length of table
        res = crs.fetchone()  # fetch the next row from the query
        print('{:<15d}{:<35s}{:<15d}{:<15f}'.format(res[0], res[1], res[2], res[3]))# print the information from the current row formatted to be aligned as a grid
        print('================================================================================')

def printBillHistory(custID):
    query = "select account_num, date, total_price from billing where customer_ID = " + str(
        custID) + ";"  # grab selected columns in the billing table
    resultCount = crs.execute(query)

    print('{:<15s}{:<15s}{:<15s}'.format("Account", "Date", "Total"))  # Section headers formatted to be in a grid
    ...
    while ct < resultCount:  # Make sure it only runs for the length of table
        ct += 1
        res = crs.fetchone()  # fetch the next row from the query
        print('{:<15d}{:<15s}{:<15f}'.format(res[0], str(res[1]), res[2])) # print the information from the current row formatted to be aligned as a grid
        print('======================================================')
...
def customer(custID):
        ...
        if (resultCount == 1):  # one result will be found if it is there
            ...
            if stock > 0:
                ...
                if quantity <= stock:
                    ...
                    total += (price * quantity)  # Add to the total taking into account the discount and number of items bought

                    query = "update items set stock = " + str(
                        stock - quantity) + " where item_ID = " + itemID + ";"  # reduce the stock of the item by the quantity bought
                    crs.execute(query)
    ...
    date = datetime.today().strftime(
        '%Y-%m-%d')  # gives today's date in year-month-day order.

    query = "insert into billing values (" + str(billID) + "," + str(custID) + "," + accountNum + ",'" + date + "'," + str(total) + " );"
    crs.execute(query)
    ...
    while ct < len(itemsBought):  # add an entry for every item selected
        ...
        query = "insert into itemsbought values (" + str(billID) + "," + itemsBought[ct] + "," + str(
            quantities[ct]) + " );"
        crs.execute(query)
        ct += 1
...
def printRecentTransaction():
    ...
    print('{:<10s}{:<15s}{:<10s}{:<15s}{:<10s}'.format("BillID", "CustomerID", "Account", "Date", "Total"))  # Section headers formatted to be in a grid
    print(
        "=========================================================================")  # Just a line to make it look pretty
